...
class VenteProduitSerializer(serializers.ModelSerializer):
    ...
    # Affiche le nom du client lors de la lecture
    client = serializers.StringRelatedField(read_only=True)

    produit_nom = serializers.CharField(source="produit.name", read_only=True)
    prix_produit = serializers.IntegerField(source="produit.prix", read_only=True)  # prix initial (juste info)

    # Prix conventionnel → OBLIGATOIRE
    prixUnitaire = serializers.IntegerField(required=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            'client_name',
            'produit_nom',
            'prix_produit',   # juste informatif
            'prixUnitaire',   # prix convenu → utilisé pour le total
            'quantite',
            'date_vente',
            'total'
        ]
